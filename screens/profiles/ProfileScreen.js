...
getData= async()=> {
  try {
      await AsyncStorage.getItem('userProfileImg').then(value => {
        ...
        if (value != null) {
          ...
          this.setState({ profileimgPath: user.uploadedImgUrl });
        }
    ...
    try {
      await AsyncStorage.getItem('userMore').then(value => {
        ...
        if (value != null) {
          let user = JSON.parse(value)
          this.setState({ about: user.jobTitle });
          this.setState({ company: user.jobCompany });
        }
        console.log(this.state.userImg);
      })
    ...
    try {
      await AsyncStorage.getItem('location').then(value => {
        ...
        if (value != null) {
          let user = JSON.parse(value)
          this.setState({ address: user.location });
    ...
    return (
      ...
      <ScrollView style={{ backgroundColor: 'white', flex: 1 }}>
        ...
        <View style={styles.nameSection}>
          ...
          <Text style={{ fontSize: 18, color: 'black' }}>{this.state.about} at {this.state.company}</Text>
  ...
  <View style={{ flex: 7,justifyContent:'center',paddingLeft:5, }}>
    ...
    <Text style={{ color: 'black', fontSize: 10 }}>Manage your connections, events, and interests</Text>
        ...
        <View style={{justifyContent:'center',alignItems:'center',width:'100%',backgroundColor:'grey',height:50}}>
